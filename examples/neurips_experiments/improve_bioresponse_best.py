"""Bioresponse: Best configuration - Decomposed structure with optimized settings."""
import sys
...
def run_bioresponse_best():
    """Bioresponse with optimized Decomposed structure."""
    print("\n" + "="*60)
    print("BIORESPONSE - Optimized Decomposed Structure")
    print("="*60)
    ...
    # Fixed top features selected on full data
    ...
    # Use 4 top features with moderate bins
    n_shape_features = 4
    n_bins = 8
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X, y)):
        ...
        # Use up to order 2 (main effects + pairwise)
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (optimized): {mean_auc:.4f} +/- {std_auc:.4f}")
    print(f"  Previous: 0.844, EBM: 0.866, LGBM: 0.872")
    return mean_auc, std_auc


if __name__ == "__main__":
    run_bioresponse_best()
