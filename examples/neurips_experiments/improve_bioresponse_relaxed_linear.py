"""Bioresponse: Relaxed linear regularization + lattice for interactions."""
import sys
...
def run_bioresponse_relaxed():
    """Bioresponse with relaxed linear regularization + lattice interactions."""
    print("\n" + "="*60)
    print("BIORESPONSE - Relaxed Linear + Lattice Interactions")
    print("="*60)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X, y)):
        ...
        # Prior variances - keep lattice regularization, RELAX linear
        prior_vars = {}
        ...
        # Much weaker regularization on beta - let linear model do more work
        # Standard ridge: tau_beta = 1.0 (weak)
        tau_beta = 1.0  # Relaxed from 0.0366
        print(f"    τ_global: {tau_global:.4f}, τ_beta: {tau_beta:.4f} (relaxed)")
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (relaxed linear): {mean_auc:.4f} +/- {std_auc:.4f}")
    print(f"  Previous: 0.844, EBM: 0.866, LGBM: 0.872")
    return mean_auc, std_auc


if __name__ == "__main__":
    run_bioresponse_relaxed()
