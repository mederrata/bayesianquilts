"""Bioresponse: Lattice-only model (no linear term)."""
import sys
...
def run_bioresponse_lattice_only():
    """Bioresponse with lattice-only model."""
    print("\n" + "="*60)
    print("BIORESPONSE - Lattice Only (No Linear Term)")
    print("="*60)
    ...
    # Use more features since we don't have linear term
    n_features = 5
    n_bins = 8
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X, y)):
        ...
        top_features = np.argsort(importance)[::-1][:n_features]
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (lattice only): {mean_auc:.4f} +/- {std_auc:.4f}")
    print(f"  Previous: 0.844, EBM: 0.866, LGBM: 0.872")
    return mean_auc, std_auc


if __name__ == "__main__":
    run_bioresponse_lattice_only()
