"""Adult v22: Add grouped occupation to lattice based on EBM analysis."""
import sys
...
def run_adult():
    print("ADULT V22 - Add grouped occupation to lattice")
    print("="*70)
    ...
    # Categorical features for one-hot (excluding those we'll handle specially)
    ...
    # Group occupation into 5 categories based on income correlation
    # High income: Exec-managerial, Prof-specialty
    # Medium-high: Tech-support, Sales
    # Medium: Craft-repair, Protective-serv, Transport-moving
    # Medium-low: Adm-clerical, Machine-op-inspct, Farming-fishing
    # Low: Other-service, Priv-house-serv, Handlers-cleaners, Armed-Forces, ?
    ...
    # Intercept lattice: 7 dimensions
    # age(6) × marital(7) × edu(4) × cg(4) × cl(2) × sex(2) × occ(5) = 13440 cells
    print(f"Intercept lattice: age(6) × marital({n_marital}) × edu(4) × cg(4) × cl(2) × sex(2) × occ({n_occ})")
    print(f"  = {6 * n_marital * 4 * 4 * 2 * 2 * n_occ} cells")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        # Add full occupation one-hot (for detailed effects)
        ...
        # Intercept lattice dimensions (7 dims with occupation group)
        dims_int = [
            Dimension("age", 6),
        ...
        # Order 2 for both - 13440 cells is manageable at order 2
        max_order_int = 2
        max_order_beta = 2
        ...
        ordinal_int = {"age", "education", "capital_gain", "occupation"}
        ordinal_beta = {"hours"}
