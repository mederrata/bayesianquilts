"""Adult v35: v30 config with uniform regularization (no theory scales)."""
import sys
...
def run_adult():
    print("ADULT V35 - v30 config with UNIFORM regularization (no theory scales)")
    print("="*70)
    ...
    print(f"Testing UNIFORM regularization")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        # Uniform regularization - no theory scales
        reg_scale = 1.0  # Uniform for all
        ...
        def loss_fn(params):
            ...
            return bce + reg_scale * l2_reg / N_train
