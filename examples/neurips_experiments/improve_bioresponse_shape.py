"""Bioresponse: Shape functions on top continuous features + full linear term."""
import sys
...
def run_bioresponse_shape():
    """Bioresponse with shape functions on top continuous features."""
    print("\n" + "="*60)
    print("BIORESPONSE - Shape Functions + Full Linear")
    print("="*60)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X, y)):
        ...
        # Select top 3 features for shape functions (nonlinear bins)
        # These capture nonlinear relationships
        n_shape_features = 3
        n_bins = 8
        top_features = np.argsort(importance)[::-1][:n_shape_features]
        print(f"    Top features for shape functions: {top_features.tolist()}")
        print(f"    Importances: {importance[top_features]}")

        # Build lattice dimensions by binning top features
        dimensions = []
        train_indices = {}
        test_indices = {}
        ...
        # Use main effects only for shape functions (no interactions in lattice)
        # Interactions are captured by the lattice cell structure
        active_components = [name for name in decomp._tensor_parts.keys() if decomp.component_order(name) <= 1]

        # Actually, let's use order ≤ 2 to capture pairwise interactions
        ...
        # Prior variances - RELAXED for shape functions
        prior_vars = {}
        for name in active_components:
            ...
            if order == 0:
                ...
                # Shape function: relax prior to allow more flexibility
                # Use larger variance (weaker regularization)
                avg_count = N_train / n_bins
                tau = bound_factor * sigma_eff / np.sqrt(avg_count)
                prior_vars[name] = (tau * 3) ** 2  # 3x larger prior variance
            else:
                # Pairwise: still need some regularization
                avg_count = N_train / (n_bins ** 2)
                tau = bound_factor * sigma_eff / np.sqrt(max(avg_count, 1))
                prior_vars[name] = (tau * 2) ** 2  # 2x larger
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (shape functions): {mean_auc:.4f} +/- {std_auc:.4f}")
    print(f"  Previous: 0.844, EBM: 0.866, LGBM: 0.872")
    return mean_auc, std_auc


if __name__ == "__main__":
    run_bioresponse_shape()
