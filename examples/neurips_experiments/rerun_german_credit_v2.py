"""Rerun German Credit with simpler lattice + more regression features."""
import sys
...
def fit_logistic_model(data, decomp, max_order=2, prior_scales=None, l1_weight=0.001, n_steps=5000):
...
def run_german_credit():
    """German Credit with 2-dim lattice + rich feature set."""
    print("\n" + "="*60)
    print("GERMAN CREDIT - V2 (2-dim lattice + features)")
    print("="*60)

    data = fetch_openml(data_id=31, as_frame=True, parser="auto")
    df = data.frame

    # Only 2 most important categorical features for lattice
    ...
    print(f"\nLattice size: {len(cat_encoders['checking_status'].classes_)} x {len(cat_encoders['credit_history'].classes_)} = {len(cat_encoders['checking_status'].classes_) * len(cat_encoders['credit_history'].classes_)} cells")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_num, y)):
        ...
        prior_scales = decomp.generalization_preserving_scales(
            noise_scale=1.0, total_n=len(train_idx), c=0.5, per_component=True
        )

        train_data = {"X": X_train_s, "y": y_train, **train_indices}
        test_data = {"X": X_test_s, "y": y_test, **test_indices}

        # Fit with order-2 (includes the checking × credit_history interaction)
        params = fit_logistic_model(
            train_data, decomp, max_order=2,
            prior_scales=prior_scales, l1_weight=0.001, n_steps=5000
        )
