"""Electricity v10: Simple lattice, rich features, no regularization."""
import sys
...
def run_electricity():
    """Electricity - simple lattice, let regression features do heavy lifting."""
    print("\n" + "="*60, flush=True)
    print("ELECTRICITY V10 - Rich Features, Simple Lattice", flush=True)
    print("="*60, flush=True)
    ...
    date_bins = np.digitize(date_vals, np.percentile(date_vals, np.linspace(0, 100, 27)[1:-1]))  # 26
    ...
    nsw_bins = np.digitize(nswprice, np.percentile(nswprice, [33, 67]))

    vicprice = X['vicprice'].values.astype(float)
    vic_bins = np.digitize(vicprice, np.percentile(vicprice, [33, 67]))
    ...
    X_numeric = np.column_stack([
        # Raw
        nswprice, vicprice, nswdemand, vicdemand, transfer, date_vals,
        # Continuous versions of bins
        day_vals / 6.0, hour_block / 5.0,
        # Interactions
        nswprice * vicprice,
        nswprice - vicprice,
        nswprice / (vicprice + 0.1),
        nswdemand * nswprice,
        vicdemand * vicprice,
        nswdemand - vicdemand,
        transfer * nswprice,
        ...
        nswdemand ** 2,
        # Log transforms
        np.log1p(np.abs(nswprice)),
        np.log1p(np.abs(vicprice)),
        np.log1p(nswdemand),
        np.log1p(vicdemand),
    ])
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_all, y)):
        ...
        # Simple intercept: date (26) × hour (6) = 156 cells
        dims_int = [
            Dimension("date", 26),
            Dimension("hour_block", 6),
        ]

        # Beta: price (3) × price (3) × day (7) = 63 cells
        dims_beta = [
        ...
        # Almost no regularization - let it fit
        scale_multiplier = 500.0
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (rich features): {mean_auc:.4f} +/- {std_auc:.4f}", flush=True)
