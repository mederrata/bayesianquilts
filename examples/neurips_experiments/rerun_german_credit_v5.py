"""German Credit: lattice on original categoricals, regress on one-hots + continuous."""
import sys
...
def fit_logistic_model(data, decomp, max_order=2, prior_scales=None, n_steps=3000):
...
def run_german_credit():
    """German Credit: lattice on categoricals, regress on one-hots + continuous."""
    print("\n" + "="*60)
    print("GERMAN CREDIT - V5")
    print("Lattice: original categoricals")
    print("Regressors: one-hot categoricals + continuous")
    print("="*60)

    data = fetch_openml(data_id=31, as_frame=True, parser="auto")
    df = data.frame

    # Key categoricals for lattice (most predictive)
    ...
    # One-hot encode ALL categoricals as regression features
    ...
    print(f"  N = {len(y)}, observations per cell ~ {len(y) / total_cells:.1f}")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_num, y)):
        ...
        # Combine one-hot + continuous as regression features
        ...
        # Build lattice from categorical indices
        dimensions = []
        train_indices = {}
        test_indices = {}
        ...
        # Weaker regularization - larger prior scales
        prior_scales = {name: 10.0 for name in decomp._tensor_parts.keys()}

        train_data = {"X": X_train_s, "y": y_train, **train_indices}
        test_data = {"X": X_test_s, "y": y_test, **test_indices}

        # Fit with order-1 (main effects only), more iterations
        params = fit_logistic_model(
            train_data, decomp, max_order=1,
            prior_scales=prior_scales, n_steps=5000
        )
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (v5): {mean_auc:.4f} +/- {std_auc:.4f}")
