"""Adult v13: age bins in beta for piecewise linear age, marital in beta for hours interaction."""
import sys
...
def run_adult():
    print("ADULT V13 - age in beta (piecewise linear), marital in beta (hours interaction)")
    ...
    age_bins = np.digitize(age, age_edges)  # 6 bins
    edu_bins = np.digitize(edu, edu_edges)  # 4 bins

    # Within-bin normalized for binned vars that are also regressors
    ...
    # Numeric features:
    # - fnlwgt, capital-loss: raw (not binned in lattice)
    # - age_norm: within-bin normalized (age binned in beta lattice)
    # - edu_norm: within-bin normalized (edu binned in intercept lattice)
    # - hours: raw continuous (NOT binned, marital in beta gives interaction)
    # - cg_norm: within-bin normalized (cg binned in intercept lattice)
    X_numeric = np.column_stack([
        df["fnlwgt"].values.astype(float),
        df["capital-loss"].values.astype(float),
        age_norm,
        edu_norm,
        hours,  # raw hours - marital in beta lattice gives different coef per marital
        cg_norm,
    ])

    print("Features: fnlwgt, capital-loss, age_norm, edu_norm, hours, cg_norm")
    print("Beta lattice: age × marital")
    print("  → age_norm coef varies by age bin (piecewise linear)")
    print("  → hours coef varies by marital (interaction)")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_numeric, y)):
        ...
        # Beta: age × marital
        # 6 × 7 = 42 cells × n_features
        # - age_norm coef varies by age bin → piecewise linear
        # - hours coef varies by marital → marital×hours interaction
        ...
        if fold_idx == 0:
            total_cells_int = 6 * n_marital * 4 * 4
            total_cells_beta = 6 * n_marital
            print(f"\n    Intercept: {total_cells_int} cells, Beta: {total_cells_beta} cells")
