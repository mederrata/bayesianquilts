"""Adult: use natural categorical structure (relationship × marital-status)."""
import sys
...
def run_adult():
    """Adult with natural categorical lattice."""
    print("\n" + "="*60)
    print("ADULT - Natural Categorical Structure")
    print("="*60)
    ...
    # Use relationship × marital-status as lattice (6 × 7 = 42 cells)
    lattice_cats = ["relationship", "marital-status"]

    # Encode categorical lattice dimensions
    encoders = {}
    lattice_data = {}
    for cat in lattice_cats:
        le = LabelEncoder()
        lattice_data[cat] = le.fit_transform(df[cat].astype(str))
        encoders[cat] = le
        print(f"  {cat}: {len(le.classes_)} categories")

    # Numeric features for linear part
    ...
    # Lattice dimensions
    dim_sizes = [len(encoders[cat].classes_) for cat in lattice_cats]
    total_cells = np.prod(dim_sizes)
    print(f"\n  Lattice: {' × '.join(lattice_cats)} = {total_cells} cells")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_numeric, y)):
        ...
        # Lattice indices
        train_lattice = {cat: lattice_data[cat][train_idx] for cat in lattice_cats}
        test_lattice = {cat: lattice_data[cat][test_idx] for cat in lattice_cats}
        ...
        # Build decomposition
        dimensions = [Dimension(cat, len(encoders[cat].classes_)) for cat in lattice_cats]
        ...
        for name in active_components:
            ...
            if order == 0:
                ...
                # Find dimension size for main effect
    ...
    print(f"  Baselines - LR: 0.907, EBM: 0.930")
