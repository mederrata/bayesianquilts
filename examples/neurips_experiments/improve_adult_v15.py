"""Adult v15: marital×hours beta (worked in v10), all features with raw+normalized."""
import sys
...
def run_adult():
    print("ADULT V15 - marital×hours beta, all features with raw+normalized")
    ...
    edu_edges = np.array([9, 11, 14])  # 4 bins
    hours_edges = np.array([35, 40, 50])  # 4 bins
    ...
    # Within-bin normalized versions
    ...
    # ALL numeric features: raw + normalized for binned vars
    X_numeric = np.column_stack([
        ...
        capital_loss,
        hours,
        # Normalized versions for binned vars
        age_norm,
        edu_norm,
        hours_norm,
        cg_norm,
    ...
    print("Numeric: age, fnlwgt, edu_num, capital_gain, capital_loss, hours + age_norm, edu_norm, hours_norm, cg_norm")
    print("Beta lattice: marital × hours (28 cells) - same as v10")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_numeric, y)):
        ...
        # Intercept: age × marital × education × capital_gain (672 cells)
        dims_int = [Dimension("age", 6), Dimension("marital", n_marital),
                    Dimension("education", 4), Dimension("capital_gain", 4)]

        # Beta: marital × hours (28 cells) - same structure as v10
        ...
        if fold_idx == 0:
            print(f"\n    Intercept: {6*n_marital*4*4} cells, Beta: {n_marital*4} cells")
