"""Bank Marketing v4: Simpler lattice (3 dims order 2) + full features."""
import sys
...
def run_bank():
    print("BANK V4 - Simpler lattice (3 dims order 2) + full features")
    print("="*70)
    ...
    # Encode categoricals for lattice - only the top 3
    ...
    print(f"\nSimpler lattice: poutcome × contact × housing, order 2")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        X_train_cat = ohe.fit_transform(df.iloc[train_idx][cat_cols].astype(str))
        X_test_cat = ohe.transform(df.iloc[test_idx][cat_cols].astype(str))
        ...
        X_train = np.concatenate([X_train_s, X_train_cat, np.stack(pairwise_train, axis=1)], axis=1)
        X_test = np.concatenate([X_test_s, X_test_cat, np.stack(pairwise_test, axis=1)], axis=1)
        ...
        # Simpler intercept lattice: poutcome, contact, housing (order 2)
        dims_int = [
        ...
        train_idx_int = np.stack([
            ...
            contact_bins[train_idx], housing_bins[train_idx],
        ], axis=-1)
        test_idx_int = np.stack([
            poutcome_bins[test_idx],
            contact_bins[test_idx], housing_bins[test_idx],
        ], axis=-1)
