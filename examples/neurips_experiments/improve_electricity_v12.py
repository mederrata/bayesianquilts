"""Electricity v12: Push for 0.93+ with full order 4."""
import sys
...
def run_electricity():
    """Electricity - push for max performance."""
    print("\n" + "="*60, flush=True)
    print("ELECTRICITY V12 - Push for 0.93+", flush=True)
    print("="*60, flush=True)
    ...
    # Same binning as v7 (winning config)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_all, y)):
        ...
        # Same structure as v7 but FULL ORDER 4
        dims_int = [
            Dimension("date", 26),
            Dimension("day", 7),
        ...
        # FULL ORDER 4 for intercept, 3 for beta
        max_order_int = 4
        max_order_beta = 3
        ...
        print(f"    Intercept: order {max_order_int}, {len(active_int)} components, {n_int_params} params", flush=True)
        print(f"    Beta: order {max_order_beta}, {len(active_beta)} components, {n_beta_params} params", flush=True)
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (full order 4): {mean_auc:.4f} +/- {std_auc:.4f}", flush=True)
