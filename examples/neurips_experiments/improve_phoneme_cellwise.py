"""Phoneme: cell-specific regression coefficients (locally linear model)."""
import sys
...
def run_phoneme_cellwise():
    """Phoneme with cell-specific regression coefficients."""
    print("\n" + "="*60)
    print("PHONEME - Cell-Specific Coefficients (Locally Linear)")
    print("="*60)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_orig, y)):
        ...
        # Bin first few features for lattice (like uci_benchmarks)
        n_threshold = 30
        max_bins = min(N_train // n_threshold, 6)
        max_dims = 4  # Use 4 dims like uci_benchmarks for N >= 5000
        n_lattice_dims = min(max_dims, n_features_orig)  # Use original features for lattice
        n_bins = max_bins

        print(f"    Lattice: {n_lattice_dims} dims × {n_bins} bins")
        ...
        for i in range(n_lattice_dims):
            col_name = f"bin_{i}"
            feature_vals = X_train[:, i]  # Use standardized features
        ...
        # KEY: param_shape=[n_features] for cell-specific coefficients
        decomp = Decomposed(interactions=interactions, param_shape=[n_features], name="beta")

        # Get prior scales using generalization_preserving_scales
        ...
        # Initialize parameters - order 2 works better
        max_order = 2
        active_components = [
        ...
        def loss_fn(params):
            ...
            # Lookup cell-specific beta coefficients
            beta = decomp.lookup_flat(train_int_idx, model_params)  # (N, n_features)
            ...
            for name, param in model_params.items():
                ...
                l2_reg += 0.5 * jnp.sum(param ** 2) / ((scale * 20) ** 2)  # Weaker reg
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (cellwise): {mean_auc:.4f} +/- {std_auc:.4f}")
    print(f"  Table: 0.937, RF: 0.961")
    return mean_auc, std_auc


if __name__ == "__main__":
    run_phoneme_cellwise()
