"""Adult v19: Race as white/black/asian/other (4 categories)."""
import sys
...
def run_adult():
    print("ADULT V19 - Race as white/black/asian/other (4 categories)")
    print("="*70)
    ...
    race_bins = np.where(race == "White", 0,
                np.where(race == "Black", 1,
                np.where(race == "Asian-Pac-Islander", 2, 3)))
    ...
    # Intercept lattice: 6 dimensions with race
    # age(6) × marital(7) × cg(4) × cl(2) × sex(2) × race(4) = 2688 cells
    print(f"Intercept lattice: age(6) × marital({n_marital}) × cg(4) × cl(2) × sex(2) × race(4)")
    print(f"  = {6 * n_marital * 4 * 2 * 2 * 4} cells")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        # Intercept lattice dimensions (6 dims with race)
        dims_int = [
            Dimension("age", 6),
