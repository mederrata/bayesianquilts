"""Adult: two-lattice approach with intercept vs beta decomposition."""
import sys
...
def run_adult():
    """Adult with two-lattice approach: richer intercept, simpler beta."""
    print("\n" + "="*60)
    print("ADULT - Two-Lattice Approach")
    print("="*60)
    ...
    # LR analysis shows:
    # - Top numeric: capital-gain (2.35), education-num (0.73)
    # - Top categoricals: marital-status (5.09), occupation (8.27), workclass (3.97), relationship (3.01)
    # - native-country has highest sum but 41 categories - too sparse

    # Numeric features (excluding fnlwgt which is sample weight)
    numeric_cols = ["age", "education-num", "capital-gain", "capital-loss", "hours-per-week"]
    X_numeric = df[numeric_cols].values.astype(np.float32)

    # Categoricals for lattice - LR shows occupation (8.27) and marital-status (5.09) are top
    # Intercept lattice: marital-status (7), occupation (14), and binned capital-gain
    # Occupation has 14 categories - can be expensive, but N is large
    # Beta lattice: marital-status only (simpler)
    lattice_cats_int = ["marital-status", "occupation"]
    lattice_cats_beta = ["marital-status"]
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_numeric, y)):
        ...
        n_top = 3  # capital-gain, education-num, hours-per-week
        pairwise_train = []
        pairwise_test = []
        ...
        print(f"    Features: {len(numeric_cols)} + {len(pairwise_train)} pairwise = {n_features}")
        ...
        # Bin capital-gain (highly predictive numeric)
        # Most values are 0, so use quantile-based bins on non-zero
        capgain_col = numeric_cols.index("capital-gain")
        n_bins = 5
        cap_train = X_train_num_s[:, capgain_col]
        cap_test = X_test_num_s[:, capgain_col]
        edges = np.percentile(cap_train, np.linspace(0, 100, n_bins + 1))
        edges = np.unique(edges)
        edges[0] = -np.inf
        edges[-1] = np.inf
        train_indices["capgain_bin"] = np.digitize(cap_train, edges[1:-1])
        test_indices["capgain_bin"] = np.digitize(cap_test, edges[1:-1])

        # Build TWO lattices
        # Intercept: marital-status (7) × occupation (14) × capgain_bin (5) = 490 cells
        # With N=39k per fold, this gives ~80 per cell on average
        dims_int = []
        for col in lattice_cats_int:
            cardinality = len(np.unique(train_indices[col]))
            dims_int.append(Dimension(col, cardinality))
        dims_int.append(Dimension("capgain_bin", len(edges) - 1))

        # Beta: marital-status (7) only
        dims_beta = []
        ...
        # Intercept: higher order (can go to 3 with 4 dims)
        # Beta: order 1 only (main effects on marital-status)
        max_order_int = 3
        max_order_beta = 1
