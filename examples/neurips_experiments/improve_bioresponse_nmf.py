"""Bioresponse: NMF for non-negative sparse molecular descriptors."""
import sys
...
def run_bioresponse_nmf():
    """Bioresponse with NMF latent factors for lattice."""
    print("\n" + "="*60)
    print("BIORESPONSE - NMF Latent Factors")
    print("="*60)
    ...
    # NMF components for lattice - 3 components × 6 bins = 216 cells
    n_nmf = 3
    n_bins = 6
    d_lat = n_nmf
    print(f"\n  NMF: {n_nmf} components × {n_bins} bins = {n_bins**d_lat} cells")
    print(f"  Bin constraint: L={n_bins} < N^(1/{d_lat}) = {N**(1/d_lat):.1f} ✓")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X, y)):
        ...
        # NMF for latent factors (data is already non-negative)
        nmf = NMF(n_components=n_nmf, init='nndsvd', max_iter=500, random_state=42)
        W_train = nmf.fit_transform(X_train)
        W_test = nmf.transform(X_test)

        reconstruction_err = nmf.reconstruction_err_
        print(f"    NMF reconstruction error: {reconstruction_err:.2f}")

        # Scale the original features for linear part
        scaler = StandardScaler()
        X_train_s = scaler.fit_transform(X_train)
        X_test_s = scaler.transform(X_test)

        # Build lattice from NMF components
        dimensions = []
        train_indices = {}
        test_indices = {}
        ...
        for i in range(n_nmf):
            ...
            edges = np.percentile(W_train[:, i], percentiles)
            train_indices[f"nmf{i}"] = np.clip(
                np.digitize(W_train[:, i], edges[1:-1]), 0, n_bins - 1
            )
            test_indices[f"nmf{i}"] = np.clip(
                np.digitize(W_test[:, i], edges[1:-1]), 0, n_bins - 1
            )
            dimensions.append(Dimension(f"nmf{i}", n_bins))
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (NMF lattice): {mean_auc:.4f} +/- {std_auc:.4f}")
