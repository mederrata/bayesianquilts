"""Adult v17: Apply all learnings - tiered features, data-driven order, per-fold selection."""
import sys
...
def run_adult():
    print("ADULT V17 - Tiered features + data-driven order + per-fold selection")
    print("="*70)
    ...
    cats = ["workclass", "education", "occupation", "relationship", "native-country"]
    ...
    # Fixed bin edges (computed on full data for consistency)
    ...
    # Intercept lattice: 5 dimensions (more manageable)
    # age(6) × marital(7) × cg(4) × cl(2) × sex(2) = 672 cells
    print(f"Intercept lattice: age(6) × marital({n_marital}) × cg(4) × cl(2) × sex(2)")
    print(f"  = {6 * n_marital * 4 * 2 * 2} cells")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_numeric, y)):
        ...
        X_train_base = np.concatenate([X_train_num, X_train_cat], axis=1)
        X_test_base = np.concatenate([X_test_num, X_test_cat], axis=1)
        y_train, y_test = y[train_idx], y[test_idx]
        N_train = len(y_train)

        # Per-fold feature selection via logistic regression
        lr = LogisticRegression(C=1.0, max_iter=1000, solver="lbfgs")
        lr.fit(X_train_base, y_train)
        abs_beta = np.abs(lr.coef_[0])

        # Tiered selection
        top_threshold = np.percentile(abs_beta, 90)
        mid_threshold = np.percentile(abs_beta, 70)
        ...
        # All features in final X (full feature set)
        n_features = X_train_base.shape[1]
        X_train = X_train_base
        X_test = X_test_base

        print(f"  Features: top={len(top_features)}, mid={len(mid_features)}, weak={len(weak_features)}")
        print(f"  LR baseline AUC: {roc_auc_score(y_test, lr.predict_proba(X_test_base)[:, 1]):.4f}")

        # Intercept lattice dimensions (5 dims)
        dims_int = [
            Dimension("age", 6),
        ...
        # Beta lattice: marital(7) × hours(4) = 28 cells
        ...
        max_order_int = compute_max_order(train_idx_int, min_samples=20)
        max_order_beta = compute_max_order(train_idx_beta, min_samples=20)
        # Cap at reasonable levels
        max_order_int = min(max_order_int, 3)
        max_order_beta = min(max_order_beta, 2)
        print(f"  Data-driven orders: intercept={max_order_int}, beta={max_order_beta}")
        ...
        # Ordinal dimensions for smoothness
        ordinal_int = {"age", "capital_gain"}  # Continuous binned dims
        ordinal_beta = {"hours"}
        ...
        def loss_fn(params):
            ...
            # Ordinal smoothness
            smooth_penalty = 0.0
            dim_names_int = [d.name for d in decomp_int._interactions._dimensions]
            for n, p in params["intercept"].items():
            ...
            for n, p in params["beta"].items():
                order = decomp_beta.component_order(n)
                if order > 0:
                    for axis, dim_name in enumerate(n.split("_")):
                        if dim_name in ordinal_beta and axis < len(p.shape) - 1 and p.shape[axis] > 1:
                            smooth_penalty += ordinal_smoothness_penalty(p, axis=axis)
