"""Adult: three categorical lattice (marital × occupation × workclass)."""
import sys
...
def run_adult():
    """Adult with marital × occupation × workclass lattice."""
    print("\n" + "="*60)
    print("ADULT - Three Categorical Lattice")
    print("="*60)
    ...
    # LR analysis shows (by sum abs coef):
    # - occupation: 8.27 (14 categories)
    # - marital-status: 5.09 (7 categories)
    # - workclass: 3.97 (8 categories)
    # - relationship: 3.01 (6 categories)
    #
    ...
    # Intercept: marital (7) × occupation (14) × workclass (8) = 784 cells
    # With N=39k per fold, ~50 per cell on average
    # Beta: marital only (7 cells)
    lattice_cats_int = ["marital-status", "occupation", "workclass"]
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_numeric, y)):
        ...
        # Intercept: order 2 (main + pairwise on 3 dims = 7 components)
        # Beta: order 1 (main effects on marital-status)
        max_order_int = 2
        max_order_beta = 1
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (three-cat lattice): {mean_auc:.4f} +/- {std_auc:.4f}")
