"""Adult v27: Add marital×occupation interaction (EBM found this important)."""
import sys
...
def run_adult():
    print("ADULT V27 - Add marital×occupation interaction")
    print("="*70)
    ...
    print(f"  Base: age(6) × marital({n_marital}) × cg(4) × cl(2) × sex(2) = {6*n_marital*4*2*2} cells")
    ...
    print(f"  Marital × occupation: {n_marital} × {n_occ} = {n_marital * n_occ} cells (NEW)")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        def loss_fn(params):
            ...
            smooth_penalty = 0.0
            for key in ["occ_hours", "edu_hours", "work_hours"]:
                for name, p in params[key].items():
