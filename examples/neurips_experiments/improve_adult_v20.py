"""Adult v20: Add education(4) to intercept lattice."""
import sys
...
def run_adult():
    print("ADULT V20 - Add education(4) to intercept lattice")
    print("="*70)
    ...
    cats = ["workclass", "occupation", "relationship", "native-country"]
    ...
    # Intercept lattice: 6 dimensions with education
    # age(6) × marital(7) × edu(4) × cg(4) × cl(2) × sex(2) = 2688 cells
    print(f"Intercept lattice: age(6) × marital({n_marital}) × edu(4) × cg(4) × cl(2) × sex(2)")
    print(f"  = {6 * n_marital * 4 * 4 * 2 * 2} cells")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        # Intercept lattice dimensions (6 dims with education)
        dims_int = [
            Dimension("age", 6),
