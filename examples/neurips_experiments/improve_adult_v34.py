"""Adult v34: v30 config with less regularization (scale_mult = 100 vs 50)."""
import sys
...
def run_adult():
    print("ADULT V34 - v30 config with less regularization (scale_mult=100)")
    print("="*70)
    ...
    print(f"Testing scale_mult=100 (was 50)")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        # KEY CHANGE: Less regularization
        scale_mult = 100.0  # was 50.0
        ...
        def loss_fn(params):
            ...
            l2_reg += 0.5 * jnp.sum(params["occ_main"]**2) / (scale_mult**2)
            l2_reg += 0.5 * jnp.sum(params["edu_main"]**2) / (scale_mult**2)
            l2_reg += 0.5 * jnp.sum(params["hours_main"]**2) / (scale_mult**2)
            l2_reg += 0.5 * jnp.sum(params["rel_main"]**2) / (scale_mult**2)
