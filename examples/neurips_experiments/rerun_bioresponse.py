"""Bioresponse: cell-varying intercept with theory-based regularization."""
import sys
...
def run_bioresponse():
    """Bioresponse with theory-based regularization."""
    print("\n" + "="*60)
    print("BIORESPONSE")
    print("="*60)
    ...
    # Bin count constraint: L < (N/p)^(1/d) for cell-varying intercept (p=1)
    # With d=2, L < sqrt(N) ≈ 61
    # With d=3, L < N^(1/3) ≈ 15
    # Use d=2 with L=8 bins each for safety
    d_lat = 2
    n_bins = 8
    print(f"\n  Lattice: {d_lat} dims × {n_bins} bins = {n_bins**d_lat} cells")
    print(f"  Bin constraint check: L={n_bins} < (N/1)^(1/{d_lat}) = {N**(1/d_lat):.1f} ✓")

    # Use top features (by LR coefficient) for lattice
    # and all features for regression
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X, y)):
        ...
        # Use LR to find top features for lattice
        ...
        # Count observations per cell for computing N^(α)
        ...
        # Initialize parameters - order 1 only
