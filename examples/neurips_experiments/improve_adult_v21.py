"""Adult v21: Use relationship instead of marital in intercept lattice."""
import sys
...
def run_adult():
    print("ADULT V21 - Use relationship instead of marital in intercept")
    print("="*70)
    ...
    cats = ["workclass", "occupation", "marital-status", "native-country"]
    ...
    # Relationship (6 categories: Husband, Not-in-family, Own-child, Unmarried, Wife, Other-relative)
    ...
    print(f"Relationship classes: {le_relationship.classes_}")
    ...
    # Intercept lattice: 6 dimensions with relationship
    # age(6) × relationship(6) × edu(4) × cg(4) × cl(2) × sex(2) = 2304 cells
    print(f"Intercept lattice: age(6) × relationship({n_relationship}) × edu(4) × cg(4) × cl(2) × sex(2)")
    print(f"  = {6 * n_relationship * 4 * 4 * 2 * 2} cells")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        X_train_base = np.concatenate([X_train_num, X_train_cat, X_train_race], axis=1)
        X_test_base = np.concatenate([X_test_num, X_test_cat, X_test_race], axis=1)

        # Add pairwise interactions for top numeric features
        top_idx = [3, 4, 0, 2, 5]  # capital_gain, capital_loss, age, edu, hours
        pairwise_train = []
        pairwise_test = []
        ...
        # Intercept lattice dimensions (6 dims with relationship)
        dims_int = [
            Dimension("age", 6),
            Dimension("relationship", n_relationship),
        ...
        # Beta lattice: relationship(6) × hours(4) = 24 cells
        dims_beta = [Dimension("relationship", n_relationship), Dimension("hours", 4)]
        ...
        test_idx_int = np.stack([
            age_bins[test_idx],
            relationship_bins[test_idx],
            edu_bins[test_idx],
            capital_gain_bins[test_idx],
            capital_loss_bins[test_idx],
            sex_bins[test_idx],
        ], axis=-1)
        train_idx_beta = np.stack([relationship_bins[train_idx], hours_bins[train_idx]], axis=-1)
        test_idx_beta = np.stack([relationship_bins[test_idx], hours_bins[test_idx]], axis=-1)

        # Use order 2 for intercept, order 2 for beta
        max_order_int = 2
        max_order_beta = 2
        ...
        ordinal_int = {"age", "education", "capital_gain"}
        ordinal_beta = {"hours"}
