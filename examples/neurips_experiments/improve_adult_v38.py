"""Adult v38: Add all EBM top interactions + larger beta lattice."""
import sys
...
def run_adult():
    print("ADULT V38 - All EBM top interactions + larger beta lattice")
    print("="*70)
    ...
    print(f"\nAll EBM top interactions + richer beta lattice (occ×hours)")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        # EBM's top interactions
        # 1. age × education (0.077)
        ...
        # 2. occupation × hours (we had this)
        ...
        # 4. relationship × capital_gain (we had this)
        ...
        # 5. marital × capital_gain (new)
        dims_mar_cg = [Dimension("marital", n_marital), Dimension("capital_gain", 5)]
        decomp_mar_cg = Decomposed(interactions=Interactions(dimensions=dims_mar_cg), param_shape=[1], name="mar_cg")

        # Beta lattice: occupation × hours (richer - wage = occ_wage × hours)
        dims_beta = [Dimension("occupation", n_occ), Dimension("hours", 8)]
        ...
        train_idx_base = np.stack([
            marital_bins[train_idx], capital_gain_bins[train_idx],
            capital_loss_bins[train_idx], sex_bins[train_idx],
        ], axis=-1)
        test_idx_base = np.stack([
            marital_bins[test_idx], capital_gain_bins[test_idx],
        ...
        train_idx_mar_cg = np.stack([marital_bins[train_idx], capital_gain_bins[train_idx]], axis=-1)
        test_idx_mar_cg = np.stack([marital_bins[test_idx], capital_gain_bins[test_idx]], axis=-1)

        train_idx_beta = np.stack([occupation_bins[train_idx], hours_bins[train_idx]], axis=-1)
        test_idx_beta = np.stack([occupation_bins[test_idx], hours_bins[test_idx]], axis=-1)

        decomps = {
            "base": decomp_base,
            "age_edu": decomp_age_edu,
            "occ_hours": decomp_occ_hours,
            "edu_hours": decomp_edu_hours,
            "rel_cg": decomp_rel_cg,
            "mar_cg": decomp_mar_cg,
            "beta": decomp_beta,
        }
        ...
        for key, decomp in decomps.items():
            ...
            if key in ["age_edu", "occ_hours", "edu_hours", "rel_cg", "mar_cg"]:
        ...
        # Main effects for variables only in interaction terms
        ...
        def loss_fn(params):
            ...
            age_edu_vals = decomp_age_edu.lookup_flat(train_indices["age_edu"], params["age_edu"])[:, 0]
            ...
            mar_vals = decomp_mar_cg.lookup_flat(train_indices["mar_cg"], params["mar_cg"])[:, 0]
            ...
            intercept = base_vals + age_edu_vals + occ_vals + edu_vals + rel_vals + mar_vals + age_main + occ_main + edu_main + hours_main + rel_main
            ...
            l2_reg = 0.0
            for key in ["base", "age_edu", "occ_hours", "edu_hours", "rel_cg", "mar_cg", "beta"]:
        ...
        mar_vals = decomp_mar_cg.lookup_flat(test_indices["mar_cg"], params["mar_cg"])[:, 0]
