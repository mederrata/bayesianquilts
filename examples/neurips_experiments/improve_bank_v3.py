"""Bank Marketing v3: Full one-hot categoricals + order 4 intercept."""
import sys
...
def run_bank():
    print("BANK V3 - Full one-hot cats + order 4 intercept")
    print("="*70)
    ...
    print(f"\nFull one-hot cats + order 4 intercept")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        # Intercept lattice: poutcome, month, contact, housing, duration (order 4)
        dims_int = [
        ...
        max_order_int = 4  # Higher order for intercept
        max_order_beta = 2
