"""Adult v18: Force order 2 + pairwise interactions on top features."""
import sys
...
def run_adult():
    print("ADULT V18 - Force order 2 + pairwise features")
    print("="*70)
    ...
    # Categorical features for one-hot (excluding those in lattice)
    ...
    # Intercept lattice: 5 dimensions
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(np.zeros(N), y)):
        ...
        # Add race and sex as one-hot (not in lattice, but as features)
        ...
        # Top 5: capital_gain, capital_loss, age, education, hours
        top_idx = [3, 4, 0, 2, 5]  # indices in X_numeric
        pairwise_train = []
        pairwise_test = []
        ...
        # Force order 2 for intercept (672 cells with ~39k samples is fine)
        max_order_int = 2
        max_order_beta = 2
