"""Electricity v13: Finer price bins (5 instead of 3)."""
import sys
...
def run_electricity():
    """Electricity - finer price bins."""
    print("\n" + "="*60, flush=True)
    print("ELECTRICITY V13 - Finer Price Bins", flush=True)
    print("="*60, flush=True)
    ...
    # FINER price bins: 5 instead of 3
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_all, y)):
        ...
        # Intercept: date(26) × day(7) × hour(6) × price(5) = 5460 cells
        dims_int = [
            Dimension("date", 26),
            Dimension("day", 7),
        ...
        # Beta: nsw(5) × vic(5) × day(7) = 175 cells
        dims_beta = [
            Dimension("nsw_price", 5),
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (5 price bins): {mean_auc:.4f} +/- {std_auc:.4f}", flush=True)
