"""Phoneme: use all 5 acoustic features with small bins."""
import sys
...
def run_phoneme():
    """Phoneme with all 5 features binned."""
    print("\n" + "="*60)
    print("PHONEME - Full 5D Lattice")
    print("="*60)
    ...
    # Use 2 top features with more bins: L < sqrt(5404) ≈ 73, so L=15 is safe
    # 2D with 15 bins = 225 cells, which is more reasonable
    d_lat = 2
    n_bins = 15
    total_cells = n_bins ** d_lat
    print(f"\n  Lattice: {d_lat} dims × {n_bins} bins = {total_cells} cells")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X, y)):
        ...
        # Use variance to find top features for lattice (no need for L1 with only 5 features)
        variances = np.var(X_train_s, axis=0)
        top_idx = np.argsort(variances)[::-1][:d_lat]
        ...
        # Initialize parameters - use order 2 for pairwise interactions
        ...
        for name in active_components:
            ...
            if order == 0:
                ...
                # Higher order: use smaller regularization (cells are smaller)
                avg_cell_count = N_train / (n_bins ** order)
                tau = bound_factor * sigma_eff / np.sqrt(max(avg_cell_count, 1))
                prior_vars[name] = tau ** 2
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (5D lattice): {mean_auc:.4f} +/- {std_auc:.4f}")
    print(f"  Previous: 0.937, RF: 0.961")
