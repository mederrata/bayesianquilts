"""Adult v6: Add age and capital-gain bins."""
import sys
...
def run_adult():
    print("ADULT V6 - Age + Capital-Gain Bins")
    ...
    age_bins = np.digitize(age, [25, 35, 45, 55, 65])  # 6 bins
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X_numeric, y)):
        ...
        le_occupation = LabelEncoder().fit(df["occupation"].astype(str))
        ...
        train_occupation = le_occupation.transform(df.iloc[train_idx]["occupation"].astype(str))
        test_occupation = le_occupation.transform(df.iloc[test_idx]["occupation"].astype(str))
        
        n_marital = len(le_marital.classes_)
        n_occupation = len(le_occupation.classes_)
        
        # Intercept: marital × occupation × age × capital_gain
        dims_int = [Dimension("marital", n_marital), Dimension("occupation", n_occupation), 
                    Dimension("age", 6), Dimension("capital_gain", 4)]
        dims_beta = [Dimension("age", 6), Dimension("hours", 4)]
        ...
        train_idx_int = jnp.stack([jnp.array(train_marital), jnp.array(train_occupation), 
                                    jnp.array(age_bins[train_idx]), jnp.array(capital_gain_bins[train_idx])], axis=-1)
        test_idx_int = jnp.stack([jnp.array(test_marital), jnp.array(test_occupation),
                                   jnp.array(age_bins[test_idx]), jnp.array(capital_gain_bins[test_idx])], axis=-1)
        train_idx_beta = jnp.stack([jnp.array(age_bins[train_idx]), jnp.array(hours_bins[train_idx])], axis=-1)
        test_idx_beta = jnp.stack([jnp.array(age_bins[test_idx]), jnp.array(hours_bins[test_idx])], axis=-1)
