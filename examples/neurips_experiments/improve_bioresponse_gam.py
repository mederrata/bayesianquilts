"""Bioresponse: GAM-like approach - many features, main effects only."""
import sys
...
def run_bioresponse_gam():
    """Bioresponse with GAM-like structure: many features, main effects only."""
    print("\n" + "="*60)
    print("BIORESPONSE - GAM-like (Many Features, Main Effects Only)")
    print("="*60)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(X, y)):
        ...
        # Select top 10 features for shape functions
        n_features_gam = 10
        n_bins = 8  # 8 bins per feature
        top_features = np.argsort(importance)[::-1][:n_features_gam]
        print(f"    Top {n_features_gam} features: {top_features.tolist()}")

        # Build dimensions by binning each feature
        dimensions = []
        train_indices = {}
        test_indices = {}
        ...
        # GAM: ONLY main effects (order 0 and 1) - no interactions
        # This gives 1 + 10*8 = 81 parameters
        ...
        total_shape_params = sum(decomp._tensor_part_shapes[name][0] for name in active_components if decomp.component_order(name) == 1)
        print(f"    Shape function params: {total_shape_params} (main effects only)")
    ...
    std_auc = np.std(aucs)
    print(f"\n  OURS (GAM-like): {mean_auc:.4f} +/- {std_auc:.4f}")
