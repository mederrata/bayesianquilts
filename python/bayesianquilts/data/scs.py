"""Sexual Compulsivity Scale data loader.

Downloads and preprocesses the SCS dataset from OpenPsychometrics.org.
10 items (Q1-Q10), 4 response categories (0-3).

Reference: Kalichman & Rompa (1995).
"Sexual sensation seeking and sexual compulsivity scales:
reliability, validity, and predicting HIV risk behavior."
Journal of Personality Assessment, 65(3), 586-601.
"""
...
item_keys = [f"Q{i}" for i in range(1, 11)]

response_cardinality = 4

_DATA_URL = "https://openpsychometrics.org/_rawdata/SCS.zip"
...
def get_data(polars_out=False, cache_dir=None):
    """Load the SCS dataset.

    Responses are shifted from 1-4 to 0-3. Values of 0 (unanswered)
    ...
    csv_path = cache_dir / "SCS" / "data.csv"

    if not csv_path.exists():
        zip_path = cache_dir / "SCS.zip"
    ...
    # Shift from 1-4 to 0-3 (0=unanswered becomes -1)
