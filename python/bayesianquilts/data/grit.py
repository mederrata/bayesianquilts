"""Duckworth Grit Scale data loader.

Downloads and preprocesses the Grit Scale dataset from OpenPsychometrics.org.
12 items (GS1-GS12), 5 response categories (0-4).
"""
...
item_keys = [f"GS{i}" for i in range(1, 13)]

response_cardinality = 5

_DATA_URL = "https://openpsychometrics.org/_rawdata/duckworth-grit-scale-data.zip"
...
def get_data(polars_out=False, cache_dir=None):
    """Load the Grit Scale dataset.

    Responses are shifted from 1-5 to 0-4. Values of 0 (missed)
    ...
    csv_path = cache_dir / "duckworth-grit-scale-data" / "data.csv"

    if not csv_path.exists():
        zip_path = cache_dir / "duckworth-grit-scale-data.zip"
    ...
    # Shift from 1-5 to 0-4 (0=missed becomes -1)
