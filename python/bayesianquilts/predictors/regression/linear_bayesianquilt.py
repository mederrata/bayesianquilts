#!/usr/bin/env python3
"""Linear regression with Bayesian quilts (regression version of logistic_bayesianquilt.py)
"""
...
class LinearBayesianquilt(QuiltedBayesianModel):
    def __init__(
        self,
        split_repr_model,
        dim_regressors,
    ...
    def create_distributions(self):
        ...
        bijectors["sigma"] = tfp.bijectors.Softplus()
            ...
            prior = self.prior_distribution.log_prob(
                {
                    "regression_model": {
                        ...
                        for k in self.regression_var_list + ["sigma"]
                    },
                    "intercept_model": {
                        k: jnp.asarray(params[k], self.dtype)
