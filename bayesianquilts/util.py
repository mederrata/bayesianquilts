...
def training_loop(
    ...
    abstract_my_tree = jax.tree_util.tree_map(
        ocp.utils.to_shape_dtype_struct, params)
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch + 1) % check_convergence_every == 0:
                ...
                if avg_epoch_loss < best_loss:
                    ...
                    if checkpoint_dir is not None:
                        ckpt = {'params': params, 'opt_state': opt_state, 'epoch': epoch, 'best_loss': best_loss}
                        checkpointer.save(f'{checkpoint_dir}/best_model_{epoch}', ckpt)
    ...
    if checkpoint_dir is not None:
        try:
            ...
            if best_checkpoints:
                latest_checkpoint = max(best_checkpoints, key=lambda x: int(x.split('_')[-1]))
