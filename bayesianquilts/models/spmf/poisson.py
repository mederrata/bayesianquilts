...
class PoissonFactorization(BayesianModel):
    ...
    def compute_scales(self, data_factory, compute_normalization=True, n=None):
        if self.scale_columns and compute_normalization:
            ...
            for batch in iter(data_factory()):
                colsums += [tf.reduce_sum(batch[self.count_key], axis=0, keepdims=True)]
                col_nonzero_Ns += [
                    tf.reduce_sum(
            ...
            colmeans = colsums / N
            colmeans_nonzero = tf.cast(colsums, tf.float64) / tf.cast(
                col_nonzero_N, tf.float64
            )
            rowmean_nonzero = tf.cast(tf.reduce_sum(colmeans_nonzero), tf.float64)
    ...
    def encode(self, x, u=None, s=None):
        ...
        if self.scale_rows:
            xi_u = (
                tf.reduce_sum(tf.cast(x, self.dtype), axis=-1, keepdims=True)
                / self.xi_u_global
            )
            z *= xi_u
        return z
...
class PoissonAutoencoder(PoissonFactorization):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        data = record["data"]
        self.column_norm_factor = 1.0

        if scale_columns:
            if column_norms is not None:
                self.column_norm_factor = tf.cast(column_norms, self.dtype)
            else:
        ...
        self.neural_network_model = DenseHorseshoe(
            self.feature_dim,
            [self.feature_dim] * encoder_layers
            + [self.latent_dim]
            + [self.feature_dim] * decoder_layers
            + [self.feature_dim],
            dtype=self.dtype,
        )
        ...
        self.surrogate_distribution = build_surrogate_posterior(
            self.joint_prior,
            self.neural_network_model.bijectors,
            dtype=self.dtype,
    ...
    def predictive_distribution(self, data, **params):
        ...
        rates = tf.math.exp(
            neural_networks(
                ...
                / tf.cast(self.column_norm_factor, self.neural_network_model.dtype)
            )
        )
    ...
    def unormalized_log_prob_parts(self, data=None, **params):
        ...
        prior_parts = self.neural_network_model.joint_prior.log_prob_parts(params)
        log_likelihood = self.log_likelihood(data, **params)
        prior_parts["x"] = log_likelihood
        return prior_parts

    def unormalized_log_prob(self, data=None, **params):
        prob_parts = self.unormalized_log_prob_parts(data, **params)
        value = tf.add_n(list(prob_parts.values()))
        return value
