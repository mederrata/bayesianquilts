...
class Transformation(ABC):
    ...
    def compute_importance_weights_helper(self,
                                          ...
                                          surrogate_log_prob_fn = None) -> Tuple[jnp.ndarray, ...]:
...
class SmallStepTransformation(Transformation):
    ...
    def normalize_vector_field(self, Q: jnp.ndarray, theta_std: jnp.ndarray = None) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        Q_norm = jnp.max(jnp.abs(Q_standardized), axis=reduction_axes, keepdims=True) # (S, N, 1)
    ...
    def __call__(self,
        ...
        # 1. Compute Q
        Q = self.compute_Q(theta, data, params, log_ell, log_pi=log_pi, log_ell_original=log_ell_original, **kwargs)
        ...
        div_Q = self.compute_divergence_Q(theta, data, params, log_ell, log_pi=log_pi, log_ell_original=log_ell_original, **kwargs)
        ...
        for i in range(N):
            ...
            if i == 0:
                for k, v in p_i.items():
                 ...
                 for k, v in p_i.items():
                    params_new[k] = jnp.concatenate([params_new[k], v[:, jnp.newaxis, ...]], axis=1)

        # Compute weights
        eta_weights, psis_weights, khat, log_ell_new = self.compute_importance_weights_helper(
            self.likelihood_fn, data, params, params_new, log_jac, variational, log_pi, log_ell_original, surrogate_log_prob_fn
        )
...
class Variance(SmallStepTransformation):
    ...
    def __init__(self, likelihood_fn: LikelihoodFunction, log_f_fn: Optional[Callable] = None):
    ...
    def compute_Q(self, 
        ...
        if log_pi is None: raise ValueError("log_pi required")
        ...
        if log_pi_broadcasted.ndim == 2 and log_pi_broadcasted.shape[1] == 1:
             log_pi_broadcasted = jnp.tile(log_pi_broadcasted, (1, current_log_ell.shape[1]))
...
class PMM1(SmallStepTransformation):
    ...
    def compute_Q(self, 
        ...
        if log_ell_original is None: raise ValueError("log_ell_original required")
        ...
        for name, value in params.items():
            ...
            if value.ndim == 1:
                ...
                # Broadcast to (S, N, K)
                diff_params[name] = jnp.tile(d[jnp.newaxis, :, :], (value.shape[0], 1, 1))
    ...
    def compute_Q(self, 
        ...
        for name, value in params.items():
            ...
            if value.ndim == 1:
                ...
                ratio = jnp.sqrt(m['var_w'] / (m['var'][jnp.newaxis, :] + 1e-10)) # N x K
                term1 = (ratio - 1.0)[jnp.newaxis, :, :] * value[:, jnp.newaxis, :]
                term2 = (m['mean_w'][jnp.newaxis, :, :] - ratio[jnp.newaxis, :, :] * m['mean'][jnp.newaxis, jnp.newaxis, :])
                Q_dict[name] = term1 + term2
...
class MM1(GlobalTransformation):
    def __call__(self, max_iter, params, theta, data, log_ell, log_pi=None, log_ell_original=None, **kwargs):
        ...
        if log_ell_original is None: raise ValueError("required")
        log_w = -log_ell_original
        weights = jnp.exp(log_w)
        moments = Transformation.compute_moments(params, weights)
        ...
        eta_weights, psis_weights, khat, log_ell_new = self.compute_importance_weights_helper(
            self.likelihood_fn, data, params, new_params, log_jac, False, log_pi, log_ell_original=log_ell_original
        )
    ...
    def __call__(self, max_iter, params, theta, data, log_ell, log_pi=None, log_ell_original=None, **kwargs):
        ...
        for name, value in params.items():
            ...
            if value.ndim == 1:
                ...
                new_params[name] = term1 + m['mean_w'][jnp.newaxis, :, :]
                log_det_jac += jnp.sum(jnp.log(ratio), axis=tuple(range(1, ratio.ndim)))[jnp.newaxis, :]
                
        theta_new = self.likelihood_fn.extract_parameters(new_params)
        
        eta_weights, psis_weights, khat, log_ell_new = self.compute_importance_weights_helper(
            self.likelihood_fn, data, params, new_params, log_det_jac, False, log_pi, log_ell_original=log_ell_original
        )
...
class AdaptiveImportanceSampler:
    ...
    def adaptive_is_loo(self, 
        ...
        if variational and self.surrogate_log_prob_fn is not None:
             ...
             grad_log_pi = jax.grad(lambda p: jnp.sum(self.surrogate_log_prob_fn(p)))(params)
        ...
        theta_expanded = theta[:, jnp.newaxis, :] # (S, 1, K) broadcastable to (S, N, K)
        ...
        if transformations is not None:
             transforms = {name: all_transforms[name] for name in transformations if name in all_transforms}
        ...
        if transformations is None or 'identity' in transformations:
            if verbose: print("Running identity...")
            # Compute identity importance weights (standard PSIS-LOO)
            # log_jacobian is 0
            log_jac_identity = jnp.zeros_like(log_ell)
            eta_weights, psis_weights, khat, log_ell_new = Transformation.compute_importance_weights_helper(
                None, self.likelihood_fn, data, params, params, log_jac_identity, variational, log_pi, log_ell, self.surrogate_log_prob_fn
            )
            ...
            best_metrics['p_loo_eta'] = jnp.where(idx, p_loo_eta, best_metrics['p_loo_eta'])
            best_metrics['p_loo_psis'] = jnp.where(idx, p_loo_psis, best_metrics['p_loo_psis'])
            ...
            best_metrics['ll_loo_eta'] = jnp.where(idx, ll_loo_eta, best_metrics['ll_loo_eta'])
            best_metrics['ll_loo_psis'] = jnp.where(idx, ll_loo_psis, best_metrics['ll_loo_psis'])
        ...
        for name, transform in transforms.items():
            ...
            current_rhos = rhos
            if name in ['mm1', 'mm2']:
                current_rhos = [1.0] # These don't use step size in the same way (or are global)
            ...
            for rho in current_rhos:
                try:
                    ...
                    best_metrics['khat'] = jnp.where(improvement, khat_safe, best_metrics['khat'])
                    best_metrics['p_loo_eta'] = jnp.where(improvement, res['p_loo_eta'], best_metrics['p_loo_eta'])
                    best_metrics['p_loo_psis'] = jnp.where(improvement, res['p_loo_psis'], best_metrics['p_loo_psis'])
                    
                except Exception as e:
                    if verbose: print(f"Failed {name} rho={rho}: {e}")
                    # import traceback
                    # traceback.print_exc()
                    continue
