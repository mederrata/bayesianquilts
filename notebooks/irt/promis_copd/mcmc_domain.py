#!/usr/bin/env python
"""Per-domain marginal MCMC for PROMIS COPD.
...
Usage:
    python mcmc_domain.py depression
    python mcmc_domain.py anxiety
    python mcmc_domain.py --all          # run all 9 domains sequentially

Available domains: depression, anxiety, anger, fatigue_experience,
...
def run_domain(domain: str):
    """Fit ADVI + MCMC for a single COPD domain."""
    print(f"\n{'='*60}")
    print(f"  PROMIS COPD — {domain}")
    print(f"{'='*60}\n")
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Domain name (e.g. depression) or --all",
    )
