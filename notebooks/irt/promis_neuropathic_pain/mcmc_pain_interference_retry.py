#!/usr/bin/env python
"""Retry NP pain_interference with larger step size."""
import os, sys
...
df, num_people = get_data(polars_out=True, domain='pain_interference')
...
# MCMC with larger step size (0.005 instead of 0.001)
# and more warmup to let the mass matrix adapt better
mcmc_samples = model.fit_marginal_mcmc(
    data, theta_grid=None,
    num_chains=4, num_warmup=3000, num_samples=500,
    thinning=5, target_accept_prob=0.85,
    step_size=0.005,  # larger initial step
...
# Save
model.save_to_disk("mcmc_pain_interference")
...
np.savez("mcmc_samples/mcmc_pain_interference.npz", **save_dict)

overall = "CONVERGED" if all_ok else "NOT CONVERGED"
print(f"\n  >>> pain_interference: {overall} <<<")
